...
export default () => {
  ...
  return (
    <header
      ref={elementRef}
      className=" fixed top-0 z-40 flex w-full items-center   justify-center  bg-white p-4 shadow-sm transition-transform duration-1000  "
    >
      <div className=" container relative flex w-11/12  items-center justify-between gap-2">
        ...
        {window.innerWidth > 768 ? (
          <nav className="flex   w-full   gap-2 text-center font-bold  text-white  ">
                  ...
                  navlinks.map(({ href, label }) => {
                    return (
                      <li key={label}>
                        <Link
                          href={href}
                          className={` flex  items-center justify-center p-2 text-sm text-grayish/70 hover:text-cyan/70 active:bg-cyan`}
                        >
                          {label}
                        </Link>
                      </li>
              ...
              <div className=" flex items-center justify-center whitespace-nowrap">
                <li className={` p-2 `}>
                  {token !== null ? (
                    ...
                    <LoginModale className=" w-full" variant="ghost">
                      log in
                    </LoginModale>
                  )}
                </li>
                ...
                <li className="w-full rounded-md p-2">
                  <SignUpModale className="w-full">sign up</SignUpModale>
                </li>
              </div>
            </ul>
          </nav>
          ...
          <Menu.Root>
            <Menu.Trigger asChild>
              <Btn className="bg-black flex  justify-center rounded-md bg-opacity-20 p-2 text-sm font-medium text-white hover:bg-opacity-30 focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 ">
                <Bars3Icon className=" h-8 w-8 stroke-gray stroke-1" />
              </Btn>
            </Menu.Trigger>

            <Menu.Content asChild>
              <nav className=" fixed top-20 left-2 right-2 z-40 mx-auto mt-2 flex  origin-top-right  flex-col gap-2 rounded-md   bg-very-dark-blue p-4 text-center font-bold  text-white shadow-lg  ring-1 ring-cyan/90 ring-opacity-5  focus:outline-transparent mn:left-8 mn:right-8 sm:left-12 sm:right-12 ">
                <ul className=" flex w-full flex-col justify-center">
                    ...
                    navlinks.map(({ href, label }) => {
                      return (
                        <li key={label}>
                          <Link
                            href={href}
                            className={`relative flex h-10 w-full items-center justify-center rounded-md p-2 text-sm hover:bg-cyan active:bg-cyan`}
                          >
                            {label}
                          </Link>
                        </li>
                  ...
                  <hr className=" mx-4 text-grayish/70 " />
