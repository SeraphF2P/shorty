...
export default () => {
  ...
  return (
    ...
    >
      <div className=" container flex w-11/12  items-center justify-between gap-2">
        ...
        <Menu as="nav" className="  inline-block text-left md:hidden">
          <div>
            <Menu.Button className="bg-black inline-flex w-full justify-center rounded-md bg-opacity-20 px-4 py-2 text-sm font-medium text-white hover:bg-opacity-30 focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 ">
          ...
          >
            <Menu.Items
              as="ul"
              className="absolute  right-0  mt-2 flex w-full  origin-top-right flex-col gap-2 rounded-md  bg-very-dark-blue p-4  text-center font-bold text-white  shadow-lg ring-1 ring-cyan/90 ring-opacity-5 focus:outline-transparent"
            >
              {navlinks &&
                navlinks.map(({ href, label }) => {
                  return (
                    <Menu.Item
                      className={`w-full rounded-md p-2 ui-active:bg-cyan`}
                      as="li"
                      key={label}
                    >
                      <Link
              ...
              >
                ...
                >
                  {({ hasStatus }: { hasStatus: boolean }) => {
                    return hasStatus ? "loged in" : "login";
                  }}
                </Btn>
              </Menu.Item>
              <Menu.Item className={`w-full rounded-md p-2 `} as="li">
                <Btn
                  className="w-full   p-2 text-center text-sm ui-active:scale-105"
                  shape="filled"
                  onClick={() => {
        ...
        >
          <Menu.Items static as="ul" className=" flex  gap-4">
            {navlinks &&
              navlinks.map(({ href, label }) => {
                return (
                  <Menu.Item as="li">
                    <Link
                      className="text-sm text-gray ui-active:outline-cyan"
                      href={href}
                    >
                      {label}
                    </Link>
          ...
          <ul className=" flex items-center  gap-4">
            <li className={` rounded-md p-2  ui-active:outline-cyan`}>
              <Btn
                onClick={loginOnClickHandeler}
                status=" border-2 px-4 py-2 rounded-full border-solid border-cyan"
                hasStatus={localStorage.getItem("token") != undefined}
